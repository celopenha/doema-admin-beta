    ...
    app.get('/app/' + rota + '/edit/:id', function (req, res) {
        if (!req.session.token) {
            ...
            request({
                ...
                //respostaBiblioteca = body.data.retornoBiblioteca[0].resposta;
                //console.log('resp = '+respostaBiblioteca);
    ...
    app.post('/app/' + rota + '/edit/submit', upload.any(), function (req, res) {
        ...
        for (let i = 0; i < Object.keys(files).length; i++) {            
            if(req.body.documentoId){
                documento.id =  Array.isArray(req.body.documentoId) == true ? req.body.documentoId[i] : req.body.documentoId;
            }            
            documento.resposta = Array.isArray(req.body.resposta) == true ? req.body.resposta[i] : req.body.resposta;
            documentoscolection.push(documento);
        }
        for (let i = 0; i < Object.keys(req.body).length; i++) {
            if(typeof documento == 'undefined' && req.body.documentoId){
                documento = {
                    "anexo": null,
                    "id":  Array.isArray(req.body.documentoId) == true ? req.body.documentoId[i] : req.body.documentoId,
