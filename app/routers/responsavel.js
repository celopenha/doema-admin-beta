...
module.exports = async function (app) {
    ...
    app.get('/app/' + rota + '/list', function (req, res) {
        ...
        if (!req.session.token) {
            ...
            request({
                url: process.env.API_HOST + rota + "/escola/" + req.session.escola,
                method: "GET",
                json: true,
                headers: {
    ...
    app.post('/app/' + rota + '/create/submit', upload.single('photo'), function (req, res) {
        ...
        request({
            ...
            if (response.statusCode != 200) {
                req.flash("danger", "Não foi possível cadastrar Responsável. " + body.errors);
            } else {
                req.flash("success", "Responsável cadastrado com sucesso.");
            }
