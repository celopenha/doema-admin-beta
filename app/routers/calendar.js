...
module.exports = async function (app) {
    ...
    app.get('/app/servico/:id/sessao/' + rota, function (req, res) {
        if (!req.session.token) {
            ...
            }, function(error, response, servico) {
                if (response.statusCode != 200 | response.statusCode != 201) {
                    request({
                        ...
                        if (!req.session.token) {
                            res.format({
                                html: function () {
                                    res.render('sessao/Calendar-public', {nome: servico.data.nome, itens: lista, eventos: sessoes, page: rota, servico: req.params.id, informacoes: req.session.json, jsonescola: req.session.jsonescola });
                                }
                            });
                            return lista;
                        } else {
                            res.format({
                                html: function () {
                                    res.render('sessao/Calendar', {nome: servico.data.nome, itens: lista, eventos: sessoes, nivelUser: req.session.json.NivelUser, page: rota, servico: req.params.id, informacoes: req.session.json, jsonescola: req.session.jsonescola });
                                }
                            });
                            return lista;
                        }
